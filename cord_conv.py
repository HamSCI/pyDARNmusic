...
def coord_conv(lon, lat, start, end, altitude=None, date_time=None,
    ...
    # from davitpy.models import aacgmv2.
    ...
    alt_conv = (end_altitude is not None and end_altitude != alt)
    ...
    if start != end or alt_conv:
        ...
        if start in aacgm_sys:
            ...
            if start == "mlt":
                ...
                mlt_0 = aacgmv2._aacgmv2.mlt_convert(date_time.year, date_time.month,
            ...
            if (end not in aacgm_sys) or alt_conv:
                lat, lon, _ = aacgmv2.wrapper.convert_latlon(lat, lon, alt,
                                                       date_time, 'A2G')
                start = "geo"
    ...
    if start != end:
        ...
        if end in aacgm_sys:
            ...
            if start == "geo":
                lat, lon, _ = aacgmv2.wrapper.convert_latlon(lat, lon, alt, date_time,
                                                       'G2A')
                start = "mag"
            ...
            if end == "mlt":
                for num, el in enumerate(lon):
                    ...
                    lon[num] = aacgmv2._aacgmv2.mlt_convert(date_time.year,
...
def get_coord_dict():
    ...
    for code, name in coord_dict.items():
        coord_string += "\n" + name + " (" + code + ")"
